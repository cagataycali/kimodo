...
#
# Qwen3 bidirectional model for LLM2Vec text encoding.
...
class ModifiedQwen3Attention(Qwen3Attention):
    """Qwen3 attention with causal masking disabled for bidirectional encoding."""
...
class ModifiedQwen3DecoderLayer(Qwen3DecoderLayer):
    """Qwen3 decoder layer using bidirectional (non-causal) attention."""
...
class Qwen3BiModel(Qwen3Model):
    """Qwen3 model with bidirectional attention (no causal mask).

    Used as text encoder for Kimodo — encodes text prompts into embeddings
    that the diffusion backbone conditions on. hidden_size=4096 matches
    Llama-3-8B exactly, making it a drop-in replacement.
    """
    ...
    def __init__(self, config: Qwen3Config):
        if not is_transformers_attn_greater_or_equal_4_43_1():
            raise ValueError(
                "Qwen3BiModel requires transformers >= 4.43.1"
            )
        ...
        # Qwen3 needs this attribute for forward() sliding window check
    ...
    def _update_causal_mask(
        ...
        # Key difference: fill with zeros (attend to everything) — bidirectional
...
class Qwen3BiForMNTP(Qwen3ForCausalLM):
    """Qwen3 bidirectional model for masked next token prediction (MNTP)."""
